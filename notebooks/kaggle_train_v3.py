...
if IS_KAGGLE:
    BASE_MODEL_PATH = Path("/kaggle/input/agrimlops-v2-artifacts/models/model_v2.pt")
    LABEL_MAP_PATH = Path("/kaggle/input/agrimlops-v2-artifacts/models/label_map.json")
    DATA_DIR = Path("/kaggle/working/plantwild")
    EXTRACT_DIR = Path("/kaggle/working/plantwild_extracted")
    ARTIFACT_BASENAME = "/kaggle/working/agrimlops_artifacts_v3"
else:
    BASE_MODEL_PATH = Path("models/model_v2.pt")
    LABEL_MAP_PATH = Path("models/label_map.json")
    DATA_DIR = Path("data/plantwild")
    EXTRACT_DIR = Path("data/plantwild_extracted")
    ARTIFACT_BASENAME = "data/agrimlops_artifacts_v3"
...
# Use same split as v1/v2 — identical SEED ensures same train/val/test sets
...
plt.title("PlantWild Subset Class Distribution (v3)")
plt.tight_layout()
...
feedback_save.to_csv(REPORT_DIR / "simulated_feedback_v3.csv", index=False)
